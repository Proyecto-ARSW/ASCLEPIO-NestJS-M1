...
@ObjectType({ description: 'Consentimiento informado registrado para un paciente' })
export class ConsentimientoPaciente {
  @Field(() => Int, { description: 'ID del consentimiento' })
  id: number;

  @Field(() => ID, { description: 'ID del paciente al que pertenece el consentimiento' })
  pacienteId: string;

  @Field({ description: 'Tipo de consentimiento (ej. "Cirugía", "Anestesia", "Tratamiento")' })
  ...
  @Field({ nullable: true, description: 'Fecha y hora de revocación (si aplica)' })
  fechaRevocacion?: Date;

  @Field({ nullable: true, description: 'Ruta o URL del documento firmado (si aplica)' })
